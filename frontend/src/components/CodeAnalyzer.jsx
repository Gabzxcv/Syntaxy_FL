...
function renderConceptIcon(iconKey, size = 11, color = 'currentColor') {

  const strokeProps = { fill: 'none', stroke: color, strokeWidth: '2.2', strokeLinecap: 'round', strokeLinejoin: 'round' };

  if (iconKey === 'dry') {

    return <svg width={size} height={size} viewBox="0 0 24 24" {...strokeProps}><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>;

  }

  if (iconKey === 'abstraction') {

    return <svg width={size} height={size} viewBox="0 0 24 24" {...strokeProps}><path d="M12 2 2 7l10 5 10-5-10-5Z"/><path d="m2 17 10 5 10-5"/><path d="m2 12 10 5 10-5"/></svg>;

  }

  if (iconKey === 'decomposition') {

    return <svg width={size} height={size} viewBox="0 0 24 24" {...strokeProps}><path d="M14.7 6.3a1 1 0 0 1 1.4 0l1.6 1.6a1 1 0 0 1 0 1.4l-7.8 7.8-3.2.8.8-3.2 7.2-7.2Z"/><path d="M12 20h9"/></svg>;

  }

  if (iconKey === 'guard') {

    return <svg width={size} height={size} viewBox="0 0 24 24" {...strokeProps}><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="m9 12 2 2 4-4"/></svg>;

  }

  return <svg width={size} height={size} viewBox="0 0 24 24" {...strokeProps}><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>;

}



function renderConceptIconHtml(iconKey, color = '#5b21b6', size = 10) {

  const p = `fill=\"none\" stroke=\"${color}\" stroke-width=\"2.2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"`;

  if (iconKey === 'dry') {

    return `<svg width=\"${size}\" height=\"${size}\" viewBox=\"0 0 24 24\" ${p}><polyline points=\"17 1 21 5 17 9\"/><path d=\"M3 11V9a4 4 0 0 1 4-4h14\"/><polyline points=\"7 23 3 19 7 15\"/><path d=\"M21 13v2a4 4 0 0 1-4 4H3\"/></svg>`;

  }

  if (iconKey === 'abstraction') {

    return `<svg width=\"${size}\" height=\"${size}\" viewBox=\"0 0 24 24\" ${p}><path d=\"M12 2 2 7l10 5 10-5-10-5Z\"/><path d=\"m2 17 10 5 10-5\"/><path d=\"m2 12 10 5 10-5\"/></svg>`;

  }

  if (iconKey === 'decomposition') {

    return `<svg width=\"${size}\" height=\"${size}\" viewBox=\"0 0 24 24\" ${p}><path d=\"M14.7 6.3a1 1 0 0 1 1.4 0l1.6 1.6a1 1 0 0 1 0 1.4l-7.8 7.8-3.2.8.8-3.2 7.2-7.2Z\"/><path d=\"M12 20h9\"/></svg>`;

  }

  if (iconKey === 'guard') {

    return `<svg width=\"${size}\" height=\"${size}\" viewBox=\"0 0 24 24\" ${p}><path d=\"M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z\"/><path d=\"m9 12 2 2 4-4\"/></svg>`;

  }

  return `<svg width=\"${size}\" height=\"${size}\" viewBox=\"0 0 24 24\" ${p}><path d=\"M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z\"/></svg>`;

}



function renderTabIcon(iconKey, size = 12, color = 'currentColor') {

  const strokeProps = { fill: 'none', stroke: color, strokeWidth: '2.3', strokeLinecap: 'round', strokeLinejoin: 'round' };

  if (iconKey === 'students') {

    return <svg width={size} height={size} viewBox="0 0 24 24" {...strokeProps}><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>;

  }

  if (iconKey === 'matrix') {

    return <svg width={size} height={size} viewBox="0 0 24 24" {...strokeProps}><rect x="3" y="3" width="8" height="8"/><rect x="13" y="3" width="8" height="8"/><rect x="3" y="13" width="8" height="8"/><rect x="13" y="13" width="8" height="8"/></svg>;

  }

  if (iconKey === 'pairs') {

    return <svg width={size} height={size} viewBox="0 0 24 24" {...strokeProps}><path d="M10 13a5 5 0 0 0 7.54.54l2.92-2.92a5 5 0 0 0-7.07-7.07L11.72 5.2"/><path d="M14 11a5 5 0 0 0-7.54-.54L3.54 13.4a5 5 0 1 0 7.07 7.07l1.67-1.67"/></svg>;

  }

  if (iconKey === 'feedback') {

    return <svg width={size} height={size} viewBox="0 0 24 24" {...strokeProps}><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>;

  }

  if (iconKey === 'code') {

    return <svg width={size} height={size} viewBox="0 0 24 24" {...strokeProps}><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>;
    ...
    return `<div class="pair-card ${cls}">
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px">
        ...
        <div>
          ...
          <div style="margin-top:6px">${concepts.map(c => `<span class="concept-tag" style="display:inline-flex;align-items:center;gap:4px">${renderConceptIconHtml(c.iconKey, c.color, 10)} ${c.label}</span>`).join('')}</div>
...
function evidenceLevel(pair) {

  const raw = pair.rawSim || 0;

  const ast = pair.astSim || 0;

  const hybrid = pair.normSim || 0;

  const minSignal = Math.min(raw, ast, hybrid);

  if (pair.similarity >= 0.85 && minSignal >= 0.45) {

    return { label: 'High Evidence', color: '#ef4444' };

  }

  if (pair.similarity >= 0.7 && minSignal >= 0.3) {

    return { label: 'Medium Evidence', color: '#f97316' };

  }

  return { label: 'Low Evidence', color: '#22c55e' };

}



function evidenceTooltip(pair) {

  const raw = (pair.rawSim || 0) * 100;

  const ast = (pair.astSim || 0) * 100;

  const hybrid = (pair.normSim || 0) * 100;
...
/**
 * Classify a line pair to determine clone type
 * Returns 'Type-1', 'Type-2', 'Type-3', or null
 */
function classifyLinePair(lineA, lineB) {
  const trimA = lineA.trim();
  const trimB = lineB.trim();
  
  // Empty or comment-only lines - no classification
  if (!trimA || !trimB) return null;
  if (trimA.startsWith('#') || trimA.startsWith('//')) return null;
  if (trimB.startsWith('#') || trimB.startsWith('//')) return null;
  
  // Type-1: Exact match (whitespace-normalized)
  if (trimA === trimB) return 'Type-1';
  
  // Get tokens for both lines
  const tokensA = lexTokens(lineA);
  const tokensB = lexTokens(lineB);
  
  if (!tokensA.length || !tokensB.length) return null;
  
  // Type-1: Token sequences match exactly
  const tokenStrA = tokensA.join('|');
  const tokenStrB = tokensB.join('|');
  if (tokenStrA === tokenStrB) return 'Type-1';
  
  // Type-2: Normalized (identifier-masked) tokens match
  const normA = normalizeTokens(tokensA);
  const normB = normalizeTokens(tokensB);
  const normStrA = normA.join('|');
  const normStrB = normB.join('|');
  
  if (normStrA === normStrB && normStrA.length > 0) return 'Type-2';
  
  // Type-3: Partial similarity (at least 50% token overlap)
  if (tokensA.length >= 3 && tokensB.length >= 3) {
    ...
    if (similarity >= 0.5) return 'Type-3';
  }
...
function CodeDiff({ codeA, codeB, labelA='Fragment A', labelB='Fragment B', language='python' }) {
  ...
  // Classify each line pair to determine clone type
  const lineClassifications = Array.from({ length: maxLen }, (_, i) => {
    const la = linesA[i] ?? '';
    const lb = linesB[i] ?? '';
    return classifyLinePair(la, lb);
  });
        ...
        <div key={pi} className="diff-pane">
          ...
          <pre className="diff-code">
            ...
            {Array.from({ length: maxLen }, (_,i) => {
              ...
              const cloneType = lineClassifications[i];

              const cloneMeta = cloneType ? CLONE_TYPE_META[cloneType] : null;

              return (

                <div 
                  key={i} 
                  className={`diff-line ${isDiff ? pane.cls : ''} ${cloneType ? 'clone-line clone-' + cloneType.toLowerCase() : ''}`}
                  style={cloneMeta ? { borderLeftColor: cloneMeta.color } : {}}
                  title={cloneMeta ? `${cloneMeta.fullName}: ${cloneMeta.description}` : ''}
                >
                  ...
                  {cloneMeta && (
                    <span className="clone-type-indicator" style={{ background: cloneMeta.color }}>
                      {cloneMeta.label}
                    </span>
                  )}
              ...
              <section className="analyzer-section">
                ...
                {analysisData&&(
                  ...
                  <div className="analysis-visual-results">
                    ...
                    {analysisData.performance_profile&&(

                      <div className="suggestions-section">

                        <h4 className="subsection-title">Performance Profile</h4>

                        <div className="clone-scores-row">

                          <span className="clone-score-tag">Extract: {analysisData.performance_profile.extract_ms ?? 0}ms</span>

                          <span className="clone-score-tag">Detect: {analysisData.performance_profile.detect_ms ?? 0}ms</span>

                          <span className="clone-score-tag">Total: {analysisData.performance_profile.total_ms ?? 0}ms</span>

                          <span className="clone-score-tag">Blocks: {analysisData.performance_profile.function_blocks ?? 0}</span>
                    ...
                    <div className="bvs-tabs">
                      ...
                      {[
                        ...
                        >
                          ...
                          <span style={{display:'inline-flex',alignItems:'center',gap:6}}>{renderTabIcon(tab.id, 12)}<span>{tab.label}</span></span>
                                  ...
                                  <div className="explain-panel">
                                    ...
                                    {pair.perfProfile&&(
                                      ...
                                      <div className="clone-scores-row" style={{marginTop:8}}>
                                        ...
                                        <span className="clone-score-tag">Compare total: {pair.perfProfile.total_ms ?? 0}ms</span>
                                    ...
                                    <div className="clone-scores-row" style={{marginTop:8}}>
                                      ...
                                      <span className="clone-score-tag" title={evidenceTooltip(pair)}>Evidence method: score gates on overall + token/AST/hybrid floors.</span>
